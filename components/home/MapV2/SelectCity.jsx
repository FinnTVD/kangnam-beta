...
export default function SelectCity({
	...
	return (
		...
		className={`${className} relative text-black w-[5vw] ml-[1vw]`}>
			<span className="cursor-pointer" onClick={() => setIsOpen(!isOpen)}>{titleCity?.title || handleTitleInit() || "Tỉnh/thành phố"}</span>
			{isOpen && (
				<div
					className="absolute bottom-0 left-0 translate-y-[110%] bg-white rounded-[0.5vw]"
				>
					{data?.map((e, index) => (
						<div
