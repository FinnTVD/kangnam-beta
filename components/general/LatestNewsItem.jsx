    ...
    return(
        <Link href={`/danh-sach-tin-tuc/${newsItem.slug}`}>
            <div className='group cursor-pointer w-full h-full bg-center bg-no-repeat bg-cover rounded-2xl shadow backdrop-blur-[39.77px] flex overflow-hidden'>
                ...
                <div className="w-[36.875vw] pt-[2.3125vw] pl-[1.875vw] pb-[2.125vw] pr-[3.3125vw] mt-auto ml-[2vw] mb-[2vw] rounded-2xl border border-white border-opacity-80 backdrop-blur-[11px]" style={{background: "linear-gradient(180deg, rgba(255, 255, 255, 0.90) 34.38%, rgba(255, 255, 255, 0.85) 100%)"}}>
                    <div className="flex">
                    ...
                    <div className="mt-[0.625vw]">
                        <div className="w-full flex flex-col">
                            <h2 className="text-den-2 title20-700-150 group-hover:text-[#D6A279] transition duration-300 line-clamp-2">{newsItem.title}</h2>
                            <span className="text-den-2 title16-400-150 mt-[0.75vw] h-[3.25vw] line-clamp-2">{newsItem.content.replace( /(<([^>]+)>)/ig, '')}</span>
                        </div>
                    </div>
                    <Button className="mt-[1.375vw]">Chi tiết</Button>
                </div>
            </div>
        </Link>
    )
