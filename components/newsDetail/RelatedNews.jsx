            ...
            >   
                {isLoadingNewsArr && 
                    listNews.map((item, index) => (
                        <SwiperSlide key={index}> 
                            <div className='w-full bg-white rounded-2xl backdrop-blur-2xl p-[1.5vw] max-md:rounded-[13px] max-md:p-[5.3vw] max-md:py-mb10 shadow-input max-md:shadow-newsDetailMb'>
                ...
                {relatedNews?.map((item, index) => (
                    <SwiperSlide key={index} className='h-auto'>
                        <NewsItem newsOtherItem={item} lang={lang}></NewsItem>
                    </SwiperSlide>
                ))}
            </Swiper>
            {isTablet && (
