...
export default function RelatedNews({t, relatedNews}) {
    ...
    return(
        <section className='px-120 mt-[3.75vw] max-md:mt-[13.3vw] max-md:px-0 max-md:mb-[16vw]'>
            <div className='flex justify-between items-end max-md:px-mb10'>
                <div>
                    <span className='sub-title max-md:title-mb10-700-150 max-md:tracking-[0.5px]'>{t.newsDetailRelated.subtitle}</span>
                    <h2 className='title56 text-den mt-[0.62vw] max-md:normal-case max-md:mt-[1vw] max-md:title-mb25-700-130 max-md:tracking-[-0.75px]'>{t.newsDetailRelated.title}</h2>
                </div>
                {!isMobile && <Button stroke='white' href={'/danh-sach-tin-tuc'} className='bg-[#D6A279] text-white border-none'>Xem thêm tin tức</Button>}
            </div>
        <Swiper
                loop={true}
                autoplay={{
